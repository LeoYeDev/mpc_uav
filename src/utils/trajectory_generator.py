...
def draw_poly(traj, u_traj, t, target_points=None, target_t=None):
    ...
    ax.plot(p_traj[:, 0], p_traj[:, 1], p_traj[:, 2], 
            label='Trajectory', linewidth=1.5, color='royalblue')

    # 设置坐标轴标签
    ax.set_xlabel(r'$p_x$ [m]', fontsize=14)
    ax.set_ylabel(r'$p_y$ [m]', fontsize=14)
    ax.set_zlabel(r'$p_z$ [m]', fontsize=14)
    
    # 关键修改：增大刻度标签字体大小（从8改为14）
    ax.tick_params(axis='both', which='major', labelsize=10)  # 修改这里
    ...
    # 调整布局（增大底部和左侧边距）
    plt.subplots_adjust(left=0.15, bottom=0.15, right=0.92, top=0.95)  # 增加边距
    plt.show()
    # p_traj = traj[:, :3]
