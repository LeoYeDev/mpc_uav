...
def plot_offline_gp_fit(gp_regressor, x_train, y_train, full_dataset_gp, cluster_to_plot, title, input_dim, output_dim):
    ...
    :param full_dataset_gp: GPDataset, 包含完整数据。
    ...
    # 注意：这里的范围仍然基于完整数据，以确保曲线完整，但绘图时会裁剪
    ...
    #            s=5, color='lightgray', alpha=0.7, label='Full Dataset')
    ...
    fig.tight_layout()
    # plt.savefig("tu.pdf", dpi=120) # 您可以取消注释来保存图像
    plt.show()
